                ...
                @foreach($bookings as $d)
                    <div class="relative mx-auto w-full">
                        <a href="#" class="relative inline-block w-full transform transition-transform duration-300 ease-in-out hover:-translate-y-2">
                            <div class="rounded-lg @if($d->status_payment == 0) bg-white @else bg-blue-400 @endif p-4 shadow">
                                ...
                                <div class="mt-4">
                                    ...
                                    <p class="line-clamp-1 mt-2 text-lg text-gray-800">{{ $d->metode_pembayaran }}/Rp. {{ number_format($d->grandTotal) }}</p>
                                    @if($d->status_payment == 0)
                                        <form class="mt-5" action="{{ route("bookingAdmin.update", $d->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" value="{{ $d->id }}" name="booking_id">
                                            <x-primary-button>Sudah dibayar</x-primary-button>
                                        </form>
                                    @endif
                                </div>
                            </div>
